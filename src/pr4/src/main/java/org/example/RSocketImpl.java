...
public class RSocketImpl extends AbstractRSocket {
    ...
    public Flux<Payload> requestStream(Payload payload) {
        return Flux.range(1, 10)  //Отдаём назад поток с цифрами от 1 до 10
    ...
    public Flux<Payload> requestChannel(Publisher<Payload> payloads) {
        return Flux.from(payloads)
                .flatMap(payload -> Flux.fromStream(payload.getDataUtf8().codePoints().mapToObj(c -> String.valueOf((char) c)) //Разбиваем входные строки на буквы
